...
inline static double G(double sigma, double x){
    if (sigma < esp)
        return 1;
    double res = (1.0 / (sigma * sqrt(2 * PI))) * pow(e, -(pow(x, 2) / (2 * pow(sigma, 2))));
    return res;
}

inline static double modlen(int dimension, ...){
    double res = 0;
    va_list args;
...
RGBQUAD **BilateralFiltering(RGBQUAD **rgb, int w, int h){
    RGBQUAD **res = (RGBQUAD**)malloc(sizeof(RGBQUAD*) * h);
    for (int i = 0; i < h; i++)
        res[i] = (RGBQUAD*)malloc(sizeof(RGBQUAD) * w);
        
    double diag = modlen(2, (double)w, (double)h);
    diag *= 0.02;
            ...
            for (int j = 0; j < w; j++){
                ...
                for (int p = -2; p <= 2; p++)
                    for (int q = -2; q <= 2; q++){
                        int xx = min(max(0, i + p), h - 1);
                        int yy = min(max(0, j + q), w - 1);
                        double dx = (xx == h - 1) ? 0 : abs(rgb[xx + 1][yy][k] - rgb[xx][yy][k]);
                        double dy = (yy == w - 1) ? 0 : abs(rgb[xx][yy + 1][k] - rgb[xx][yy][k]);
                        gradsum += dx + dy;
                    }
                gradsum /= 25;
                for (int p = -2; p <= 2; p++)
                    for (int q = -2; q <= 2; q++){
                        ...
                        int yy = min(max(0, j + q), w - 1);
                        double coef = 
                            G(diag, modlen(2, (double)p, (double)q)) * G(gradsum, modlen(
                                3,
                                rgb[i][j][RGBA_B] - rgb[xx][yy][RGBA_B],
                                rgb[i][j][RGBA_G] - rgb[xx][yy][RGBA_G],
                                rgb[i][j][RGBA_R] - rgb[xx][yy][RGBA_R]
                            ));
                        
                        sumcoef += coef;
                        sum += coef * (double)rgb[xx][yy][k];
                    }
                sum /= sumcoef;
                res[i][j][k] = min(255, sum);
            }
    }
